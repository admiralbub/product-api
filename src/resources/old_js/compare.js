...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener('click', function(event) {
        ...
        const parentAddCompareList = event.target.closest('.AddCompareList');
        
        
        if (parentAddCompareList) {
            const AddCompareList = parentAddCompareList.querySelector('.button_scale_card');
        
            const dataAttributes = AddCompareList.dataset;
            const id = dataAttributes.id;
            
            const asyncAddCompareList = async () => {
                try {
                    const response = await axios.post('/compare/add', {
                        id: id,
                    });
