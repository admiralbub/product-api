...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.addEventListener('click', function(event) {
        ...
        const parent = event.target.closest('.AddWislistList');
        
        
        if (parent) {
            const AddWislistList = parent.querySelector('.button_heart_card');;
            const dataAttributesAddWislistList = AddWislistList.dataset;
            const id = dataAttributesAddWislistList.id;
                
            const asyncAddWislistList = async () => {
                let isAuth = dataAttributesAddWislistList.auth;
                if(isAuth == 1) {
                    try {
                        const response = await axios.post('/wislist/add', {
                            id: id,
