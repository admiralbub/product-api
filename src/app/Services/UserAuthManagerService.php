...
class UserAuthManagerService implements UserAuthManagerInterface {
    ...
    static public function authGoogleCallback() : void {
        try {
        ...
        if(!$user)
        {
            $user = User::create(['first_name' => $googleUser->name, 'last_name' => '...', 'phone' => rand(100000,999999), 'email' => $googleUser->email, 'password' => \Hash::make(rand(100000,999999))]);
        }
